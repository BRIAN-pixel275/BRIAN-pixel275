```
$ whoami
```

# Brian Muchiru

```bash
role      : Software Developer
studying  : Economics & Finance @ Kenyatta University (Class of 2028)
founder   : Kreative Studio — design & dev services, Nairobi
focus     : Finance systems | POS platforms | PWAs
status    : Learning Java, backend architecture, databases
```

![Portfolio](https://img.shields.io/badge/Portfolio-brianportfolio--eight.vercel.app-000000?style=flat-square&logo=vercel&logoColor=white)
![LinkedIn](https://img.shields.io/badge/LinkedIn-Brian_Muchiru-0A66C2?style=flat-square&logo=linkedin&logoColor=white)
![Email](https://img.shields.io/badge/Email-brayo2933%40gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white)

---

## `$ cat about.md`

I'm an Economics & Finance student building software at the intersection of finance, technology, and entrepreneurship. Most of what I build turns a real financial problem — a club treasury, a student budget, an SME's lack of credit history, an informal savings group — into a working product. Kreative Studio is where the design side of what I do (branding, UI/UX) meets the engineering side.

---

## `$ ls stack/`

**Frontend**

![React](https://img.shields.io/badge/-React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/-Vite-646CFF?style=flat-square&logo=vite&logoColor=white)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![Tailwind](https://img.shields.io/badge/-Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

**Backend & Data**

![Java](https://img.shields.io/badge/-Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Firebase](https://img.shields.io/badge/-Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black)
![Supabase](https://img.shields.io/badge/-Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![SQLite](https://img.shields.io/badge/-SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)

**Tools**

![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/-GitHub-181717?style=flat-square&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/-VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Netlify](https://img.shields.io/badge/-Netlify-00C7B7?style=flat-square&logo=netlify&logoColor=white)
![Vercel](https://img.shields.io/badge/-Vercel-000000?style=flat-square&logo=vercel&logoColor=white)

**Design**

![Photoshop](https://img.shields.io/badge/-Photoshop-31A8FF?style=flat-square&logo=adobephotoshop&logoColor=white)
![Canva](https://img.shields.io/badge/-Canva-00C4CC?style=flat-square&logo=canva&logoColor=white)

---

## `$ ls projects/ --sort=created`

<!-- PROJECTS:START -->
<!-- This section is updated automatically by .github/workflows/update-readme.yml — don't edit by hand -->
| Project | Description | Language | ⭐ |
|---|---|---|---|
| [**trace-finder**](https://github.com/BRIAN-pixel275/trace-finder) | TraceFinder is a Java command-line application that analyzes access logs using a CSV-based rulebook. It identifies normal activity, flags entries with higher severity levels, detects unknown activity patterns, and reports malformed log lines. | Java | 0 |
| [**kukisa-registration**](https://github.com/BRIAN-pixel275/kukisa-registration) | A mobile-first web application designed to help the Kenyatta University Kiambu Students Association (KUKISA) register and collect information from new students from Kiambu County. | JavaScript | 1 |
| [**creditscoreAfrica**](https://github.com/BRIAN-pixel275/creditscoreAfrica) | — | — | 0 |
| [**portfolio**](https://github.com/BRIAN-pixel275/portfolio) | portfolio website for a software engineer | CSS | 0 |
| [**elevara-legacy**](https://github.com/BRIAN-pixel275/elevara-legacy) | A modern, responsive women empowerment platform built with React, Vite, and Supabase, featuring programs, community engagement, events, contact management, and an admin dashboard. | JavaScript | 1 |
| [**HospitalManagementSystem**](https://github.com/BRIAN-pixel275/HospitalManagementSystem) | A full-stack hospital management system for managing data and patients records | Java | 0 |
| [**Brian-Muchiru-Portfolio**](https://github.com/BRIAN-pixel275/Brian-Muchiru-Portfolio) | A premium developer portfolio built with React, Tailwind CSS, and Framer Motion. | JavaScript | 0 |
| [**kood-java**](https://github.com/BRIAN-pixel275/kood-java) | everything learnt in Kood/Nairobi selection sprint 2026 | Java | 0 |
<!-- PROJECTS:END -->

---

## `$ tail -f learning.log`

```
[LEARNING]  Java, backend architecture, PostgreSQL, Docker, system design
[BUILDING]  ClubVault v2, Kreative Studio client work
[GOAL]      Ship FinTech tools Kenyan students and small businesses actually use
```

---

## `$ cat principles.txt`

```
+ Build for a real user, not a resume line
+ Ship a complete version before polishing it
+ Keep the codebase legible to future-me
+ Finish what I start
```

---

## `$ contact --all`

![Portfolio](https://img.shields.io/badge/Portfolio-brianportfolio--eight.vercel.app-000000?style=flat-square&logo=vercel&logoColor=white)
![LinkedIn](https://img.shields.io/badge/LinkedIn-Brian_Muchiru-0A66C2?style=flat-square&logo=linkedin&logoColor=white)
![Email](https://img.shields.io/badge/Email-brayo2933%40gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white)

Open for freelance web development, FinTech collaborations, and client projects.

---

```
> "Practice doesn't just make perfect — it builds experience one project at a time."
```

<sub>Last synced: <!-- LAST_UPDATED:START -->2026-09-18 20:10 UTC<!-- LAST_UPDATED:END --></sub>
